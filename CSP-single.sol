    ...
    Ownable
{
    ...
    mapping(address => mapping(uint => tokenForSell)) public listOfTokenSell;
    mapping(address => mapping(address => mapping(uint => uint)))public addBytokenByprice;
   // mapping(address => mapping(uint => order)) public auctionDetails; 
    ...
    uint256 public _tid;
    uint public platFromFee;
    address feeCollector;
    address feeCollector1;

    constructor(uint256 id, uint256 _serviceValue)
        BEP721("CloseSphere Single", "CLS")
    {
        _tid = id;
        serviceValue = _serviceValue;
    }

    function newPlatformFee(uint fee) external onlyOwner{
        platFromFee = fee.mul(100).div(1000);
    }

    function setFeeCollector(address setfeeCollector) external onlyOwner {
        feeCollector = setfeeCollector;
    }

    function serFeeCollector(address setFeeCollector1) external onlyOwner {
        feeCollector1 = setFeeCollector1;
    }
    ...
    function mintForBundle(address seller,
        address buyer,
        uint royal,
        uint amount,
        uint tokenId,
        ...
        _creator[tokenId] = seller;
        _royal[tokenId] = royal;
        token_id[tokenId] = Metadata(name, ipfsimage, ipfsmetadata);
        balances[tokenId][buyer] = supply;
        uint fee = amount.mul(platFromFee).div(100);
        uint finalAmount = amount.sub(fee);
        payable(seller).transfer(finalAmount);
    }
    ...
    //         string memory name,
    //         string memory ipfsimage,
    //         string memory ipfsmetadata) public payable{
    //     require(msg.value >= amount, "invalid price");
    ...
    function putOnSell(address nftContract, uint tokenId, uint price) external {
        listOfTokenSell[nftContract][tokenId] = tokenForSell(price, false, msg.sender, address(0));
        emit OrderPlace(msg.sender, tokenId, price);
    }

    function changeSellPrice(address nftContract, uint tokenId, uint price) external {
        require(listOfTokenSell[nftContract][tokenId].sold == false ,"already sold or remove");
        require(listOfTokenSell[nftContract][tokenId].seller == msg.sender, "you are not owner");
        listOfTokenSell[nftContract][tokenId].price = price;
    }

    function buyNFT(address nftContract, address from,uint tokenId, uint price) external payable{
        require(price == msg.value, "please pay correct price");
        require(msg.value >= listOfTokenSell[nftContract][tokenId].price);
        require(listOfTokenSell[nftContract][tokenId].sold == false, "already nft remove or sold");
        listOfTokenSell[nftContract][tokenId].sold = true;
        listOfTokenSell[nftContract][tokenId].buyer = msg.sender;

        uint fee = price.mul(platFromFee).div(100);
        uint amountOfSplit = fee.div(2);
        uint finalAmonut = price.sub(fee);
        payable(listOfTokenSell[nftContract][tokenId].seller).transfer(finalAmonut);
        payable(feeCollector).transfer(amountOfSplit);
        payable(feeCollector1).transfer(amountOfSplit);
        // token.transferFrom(msg.sender, listOfTokenSell[nftContract][tokenId].seller, price);
        IBEP721(nftContract).transferFrom(from, msg.sender, tokenId);
    }
    
    function removeFromSell(address nftContract, uint tokenId) public {
        require(listOfTokenSell[nftContract][tokenId].sold == false, "your NFT sold");
        require(listOfTokenSell[nftContract][tokenId].seller == msg.sender, "you are not owner");
        listOfTokenSell[nftContract][tokenId] = tokenForSell(0, false, address(0), address(0));

        //IBEP721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        emit CancelOrder(msg.sender, tokenId);
    }

    function setForAuction(address nftContract, uint tokenId, uint miniPrice) public {
        listOfTokenSell[nftContract][tokenId] = tokenForSell(miniPrice, false, msg.sender, address(0));
    }


    function acceptOffer(string memory tokenAss, address nftContract, address bidAdd, uint tokenId, uint amount) public {
        require(listOfTokenSell[nftContract][tokenId].sold == false, "already nft remove or sold");
        require(listOfTokenSell[nftContract][tokenId].price <= amount, "please check Price");

        listOfTokenSell[nftContract][tokenId].sold = true;
        listOfTokenSell[nftContract][tokenId].buyer = bidAdd;
        uint fee = amount.mul(platFromFee).div(100);
        uint amountOfSplit = fee.div(2);
       //uint royalty = amount.mul(_royal[tokenId]).div(100);
        uint payableAmount = amount.sub(fee);

        BEP20 t = BEP20(tokentype[tokenAss]);

        t.allowance(bidAdd, address(this));

        t.transferFrom(bidAdd, feeCollector, amountOfSplit);
        t.transferFrom(bidAdd, feeCollector1, amountOfSplit);
        t.transferFrom(bidAdd, msg.sender, payableAmount);

        IBEP721(nftContract).transferFrom(msg.sender, bidAdd, tokenId);
    }
    ...
    function SalewithToken(
        ...
        uint amountOfSplit = fee.div(2);
        uint royalty = amount.mul(_royal[tokenId]).div(100);
        uint payableAmount = amount.sub(fee.add(royalty));
        ...
        t.transferFrom(bidaddr, feeCollector, amountOfSplit);
        t.transferFrom(bidaddr, feeCollector1, amountOfSplit);
        t.transferFrom(bidaddr, _creator[tokenId], royalty);
        t.transferFrom(bidaddr, msg.sender, payableAmount);
    ...
    function saleTokenwithToken(
        ...
        newtokenasbid(tokenAss, from, feeCollector, amount, tokenId);
    ...
    function newtokenasbid(
        ...
        t.transferFrom(msg.sender, feeCollector, _adminfee);
    ...
    function _acceptBId(
        ...
        t.transferFrom(from, feeCollector, amountOfSplit);
        t.transferFrom(from, feeCollector1, amountOfSplit);
        t.transferFrom(from, _creator[tokenId], royalty);
        t.transferFrom(from, msg.sender, payableAmount); 
    }
    ...
    function _saleToken(
        ...
        payable(feeCollector).transfer(amountOfSplit);
        payable(feeCollector1).transfer(amountOfSplit);
        create.transfer(royalty);
        from.transfer(payableAmount);
    }
