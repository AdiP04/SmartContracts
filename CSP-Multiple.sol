...
contract ClosesphereMultiple is BEP1155, Ownable, BEP1155Burnable {
    event Approve(
        address indexed owner,
    ...
    function putOnSell(address nftContract, uint tokenId, uint noOfToken, uint price) external {
        uint noOftokenBalance = IBEP1155(nftContract).balanceOf(msg.sender, tokenId);
        require(noOftokenBalance >= noOfToken, "insufficient balance");
        listOfTokenSell[nftContract][tokenId] = tokenForSell(price, noOfToken, false, msg.sender, address(0));
    ...
    function buyNFT(address nftContract, address from,uint tokenId, uint noOfToken, uint price) external payable{
        require(listOfTokenSell[nftContract][tokenId].noOfToken >= noOfToken, "select correct amount");
        require(msg.value == noOfToken.mul(listOfTokenSell[nftContract][tokenId].price), "please pay correct price");
        
        require(listOfTokenSell[nftContract][tokenId].sold == false, "already nft remove or sold");
        listOfTokenSell[nftContract][tokenId].noOfToken -= noOfToken;
        //listOfTokenSell[nftContract][tokenId].price -= msg.value;
        if (listOfTokenSell[nftContract][tokenId].noOfToken == 0){
        ...
        IBEP1155(nftContract).safeTransferFrom(from, msg.sender, tokenId, noOfToken, "");
    }

    function setForAuction(address nftContract, uint tokenId, uint noOfToken, uint miniPrice) public {
        uint noOftokenBalance = IBEP1155(nftContract).balanceOf(msg.sender, tokenId);
        require(noOftokenBalance >= noOfToken, "insufficient balance");
        listOfTokenSell[nftContract][tokenId] = tokenForSell(miniPrice, noOfToken, false, msg.sender, address(0));
    }
    ...
    function removeFromSell(address nftContract, uint tokenId) public {
        ...
        require(listOfTokenSell[nftContract][tokenId].seller == msg.sender, "you are not owner");
        listOfTokenSell[nftContract][tokenId] = tokenForSell(0, 0,false, address(0), address(0));

        
        emit CancelOrder(msg.sender, tokenId);
    }

    function acceptOffer(string memory tokenAss, address nftContract, address bidAdd, uint tokenId, uint noOfToken,  uint amount) public {
        require(listOfTokenSell[nftContract][tokenId].noOfToken >= noOfToken, "select correct amount");
        //require(amount >= noOfToken.mul(listOfTokenSell[nftContract][tokenId].price), "please pay correct price");
        require(listOfTokenSell[nftContract][tokenId].sold == false, "already nft remove or sold");
       ...
       //uint royalty = amount.mul(_royal[tokenId]).div(100);
        ...
        IBEP1155(nftContract).safeTransferFrom(msg.sender, bidAdd, tokenId, noOfToken, "");
    }
    ...
    function saleWithToken(
        ...
        _saleToken(from, tokenId, amount, nooftoken);
    ...
    function _saleToken(
       ...
       // uint256 val = pBEPent(amount, serviceValue).add(amount);
            ...
            require(msg.value == order_place[from][tokenId].price.mul(nooftoken), "Insufficient Balance");
    ...
    function acceptBId(
        ...
        _acceptBId(bidtoken, bidaddr, amount, tokenId, feeCollector);
